...
class LogFileService {
  ...
  Future<File?> getLogFile() async {
    ...
    final List<File> todayLogFiles = logFiles.where((file) => file.path.contains(_today)).toList();
  ...
  Future<List<File>> getAllLogFiles() async {
    ...
    final List<File> logFiles =
        files.whereType<File>().where((file) => file.path.endsWith('.txt')).toList();
...
''';
  ...
  int _getHighestNumber(List<File> todayLogFiles) {
    ...
    todayLogFiles
        .removeWhere((file) => file.path.split(Platform.pathSeparator).last.contains('_') == false);
